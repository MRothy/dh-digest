...
- After a successful run, commits updated `seen_items.json` back to `main`; runs `git pull --rebase origin main` before pushing to handle diverged-branch failures
...
- **GitHub Actions commit step** — adds `git pull --rebase origin main` before `git push` to prevent push rejections when the remote has moved ahead (e.g. a concurrent workflow run committed first). Note: `git push --rebase` is not a valid git flag — the rebase option belongs on `git pull`.
